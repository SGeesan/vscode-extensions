...
export async function executeSingleTestCase(useCase: TestUseCase): Promise<TestCaseResult> {
    ...
    try {
        ...
        // Step 2: Set up event handler
        const { handler: testEventHandler, getResult } = createTestEventHandler(useCase);

        // Step 3: Prepare generation parameters with isolated project path
        ...
        // Step 5: Extract final state from temp project path (where AI actually made changes)
        ...
        return await validateTestResult(result, useCase, finalSources);
        ...
        // Step 7: Always cleanup both isolated project and AI temp project
