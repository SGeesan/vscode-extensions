...
export const httpUpdateTestCases: Array<TestCaseDefinition> = [
    {
        // Fix-only request — compile repair without runtime checks
        prompt: "Please fix the compile errors in this existing order service project. Keep the current API shape and business intent the same.",
        projectPath: "simple_order_management_system_with_compile_errors",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 0, maxCalls: 0 }]])
    },
    {
        // Fix with re-evaluation — compile fix + endpoint contract check
        prompt: "The order service currently has compile errors. Fix them first. Re-check behavior by sending requests.",
        projectPath: "simple_order_management_system_with_compile_errors",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 1, maxCalls: 6 }]])
    },
    {
        // Fix with re-evaluation — add lightweight diagnostics endpoint and verify
        prompt: "Please fix and validate the service.",
        projectPath: "simple_order_management_system_with_compile_errors",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 1, maxCalls: 6 }]])
    },
    {
        // Fix with re-evaluation — compile fix + CORS/validation smoke check
        prompt: "Try out the serivce. Fix if needed.",
        projectPath: "simple_order_management_system_with_compile_errors",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 1, maxCalls: 6 }]])
    }
];
...
export const httpValidateTestCases: Array<TestCaseDefinition> = [
    {
        // Validate-only — no code edits, validate known bad-request contract
        prompt: "Please validate the existing order endpoint behavior without changing any source code. Call POST /v1/orders with a realistic payload that intentionally contains an empty `orderLines` array and confirm the API returns the expected bad-request message.",
        projectPath: "simple_order_management_system",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 1, maxCalls: 6 }]])
    },
    {
        // Validate-only — route/method surface check
        prompt: "I only need endpoint validation, not code edits. Please run HTTP checks to confirm the service functions as expected.",
        projectPath: "simple_order_management_system",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 1, maxCalls: 6 }]])
    },
    {
        // Validate-only — CORS verification for existing resources
        prompt: "Please validate the current CORS behavior on this service. Confirm allowed methods/origins are honored for the configured resources.",
        projectPath: "simple_order_management_system",
        toolCallThresholds: new Map([["curlRequest", { minCalls: 1, maxCalls: 6 }]])
    }
];
