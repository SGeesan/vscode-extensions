...
function calculateAggregatedToolEvalResults(results: readonly UsecaseResult[]): AggregatedToolEvalResult[] | undefined {
    ...
    return Array.from(toolMap.entries()).map(([toolName, evalResults]) => {
        ...
        // Average only across cases where this tool was actually called.
        // No-call cases are non-applicable for success-rate math and should not count as 0.
        const applicableResults = evalResults.filter((current) => current.totalCalls > 0);
        const applicableCaseCount = applicableResults.length;
        const successRateSum = applicableResults.reduce((sum, current) => {
            return sum + (current.successCount / current.totalCalls);
        }, 0);
        const averageSuccessRate = applicableCaseCount > 0 ? successRateSum / applicableCaseCount : 0;
